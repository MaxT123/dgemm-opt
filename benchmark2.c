...
{
  printf ("SHELL CODE Description:\t%s\n\n", dgemm_desc);

  /* Test sizes should highlight performance dips at multiples of certain powers-of-two */
  
  double aveper_store[100];
  double max_aveper=0;
  int max_aveper_block_size=1, min_size=41, max_size=41;
  for (int block_size = min_size; block_size <= max_size; block_size ++)
  {
  printf("Block size: %d\n", block_size);
  ...
  aveper_store[block_size] = aveper;
  if (aveper>max_aveper)
      max_aveper = aveper, max_aveper_block_size = block_size;

  free(buf);
  }
  
  for (int block_size=min_size; block_size<=max_size; block_size++)
      printf("Block_size = %d, aveper = %g\n", block_size, aveper_store[block_size]);
  printf("Max average percentage of Peak = %g, block_size = %d\n",max_aveper, max_aveper_block_size);
